...
class DiscordBot(commands.Bot):
    ...
    async def on_message(self, message):
        ...
        await self.trafficMonitor(message)  # Traffic Monitor function call on each message
        await self.process_commands(message)

    async def trafficMonitor(self, message):   # Traffic Monitor function
        inappropriate_keywords = ["spam", "inappropriate", "kill", "murder", "drugs"]
        if any(keyword in message.content for keyword in inappropriate_keywords):
            print("Inappropriate content detected!")
            await message.channel.send("Potential spam or inappropriate content detected.")
